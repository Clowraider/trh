...
- [x] Slice B GREEN fixed prompt, compact serializer, 48,000-byte guard, isolated OpenAI-compatible client, response validator, and server-order joining.
...
- `editor_jefe_ia.py` — strict maximum parser, immutable policy, compact/budgeted payload, dedicated OpenAI-compatible adapter, all-or-nothing validator, and synchronous selection orchestration.
...
- OpenAI-compatible provider uses the `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and `OPENAI_MODEL` environment variables, a single model, JSON-object response mode, timeout 70, and response allowance 1,200 through a dedicated adapter that does not import `publicador`.
